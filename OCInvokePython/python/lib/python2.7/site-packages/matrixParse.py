...
def parse_app_info(report):
    ...
    try:
        if app_stats.get('app_launch_time'):
            app_stats['app_launch_time'] = app_stats.get('app_launch_time')
            # datetime.fromtimestamp(app_stats.get('app_launch_time')).strftime("%Y-%m-%d %H:%M:%S")
...
#        parser.print_help()
#        sys.exit(1)
    reports = json.load(open(sys.argv[0]))
